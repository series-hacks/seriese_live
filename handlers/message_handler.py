...
# SIMULATION MODE - Set to True to simulate second user's phone
# When enabled:
# - Simulated user auto-joins waitroom when you text "call"
# - Simulated user auto-accepts the match
# - Only your phone gets called (simulated phone is skipped)
# - AI features still work (intro + icebreakers play to your call)
SIMULATION_MODE = False
SIMULATED_PHONE = os.getenv('SIMULATED_PHONE', '+10000000000')
REAL_PHONE = os.getenv('REAL_PHONE', '+10000000001')

# User profiles (loaded from environment or defaults)
USER_PROFILES = {
    os.getenv('USER1_PHONE', '+10000000001'): {
        'name': os.getenv('USER1_NAME', 'User 1'),
        'title': os.getenv('USER1_TITLE', ''),
        'bio': os.getenv('USER1_BIO', ''),
        'interests': os.getenv('USER1_INTERESTS', '')
    },
    os.getenv('USER2_PHONE', '+10000000000'): {
        'name': os.getenv('USER2_NAME', 'User 2'),
        'title': os.getenv('USER2_TITLE', ''),
        'bio': os.getenv('USER2_BIO', ''),
        'interests': os.getenv('USER2_INTERESTS', '')
    }
}
...
COMMANDS = {
    ...
    'summary': 'Get summary of your last call',
...
class MessageHandler:
    ...
    def __init__(self, series_api: "SeriesAPI", twilio_service: "TwilioService"):
        ...
        # Initialize notetaker for summary generation
        self.notetaker = Notetaker(os.getenv('OPENAI_API_KEY', ''))
    ...
    def process_message(self, phone: str, text: str, chat_id: str) -> str | list[str]:
        ...
        if text_lower == 'summary':
            return self.handle_summary_request(phone, chat_id)
    ...
    def initiate_call(self, match_id: str) -> str:
        ...
        # If webhook server is configured, let it handle the calls
        if webhook_url:
            try:
                # Build profiles list for the webhook server
                profiles_list = []
                for phone in phones:
                    ...
                    profile['phone'] = phone  # Add phone field for webhook server
                    profiles_list.append(profile)

                # Call webhook server to register conference state (for intro/icebreakers)
                # Pass the conference_name so webhook uses the same name as TwilioService
                response = requests.post(
                    f"{webhook_url}/conference/create",
                    json={
                        'conference_name': conference_name,
                        'profile1': profiles_list[0],
                        'profile2': profiles_list[1]
                    },
                    timeout=10
                )
                if response.ok:
                    logger.info(f"Conference state registered with webhook server for {conference_name}")
                    # Don't return early - we still need to make the calls below
                else:
                    logger.warning(f"Webhook failed ({response.status_code}): {response.text}")
                    # Continue to make calls anyway - just won't have intro/icebreakers

            except Exception as e:
                logger.error(f"Failed to contact webhook server: {e}")
                # Fall through to direct Twilio calls below
    ...
    def handle_summary_request(self, phone: str, chat_id: str) -> str:
        """
        Handle summary request - fetch latest Twilio recording and summarize.
        ...
        """
        logger.info(f"Summary request from {phone}")

        try:
            # Get the most recent recording from Twilio API
            recordings = self.twilio.client.recordings.list(limit=1)

            if not recordings:
                return "No recordings found. Make a call first, then request a summary!"

            recording = recordings[0]
            logger.info(f"Found recording: {recording.sid}, duration: {recording.duration}s")

            # Check if recording has enough duration (at least 5 seconds)
            if int(recording.duration or 0) < 5:
                return "Recording too short to summarize. Have a longer conversation and try again!"

            # Build recording URL
            recording_url = f"https://api.twilio.com/2010-04-01/Accounts/{os.getenv('TWILIO_ACCOUNT_SID')}/Recordings/{recording.sid}"
            ...
            # Get user's profile and try to find the other participant
            my_profile = USER_PROFILES.get(phone, {'name': 'You'})
            other_profile = {'name': 'Other participant'}

            # Try to find other participant from USER_PROFILES
            for other_phone, profile in USER_PROFILES.items():
                if other_phone != phone:
                    other_profile = profile
                    break

            # Process recording: download, transcribe, summarize
            logger.info(f"Processing recording {recording.sid}...")
            summary = self.notetaker.process_recording(
                recording_url,
                twilio_auth,
                my_profile,
                other_profile
            ...
            if summary:
                return f"Here's your call summary:\n\n{summary}"
            else:
                return "Could not generate summary. The recording may be too short or unclear."
